...
namespace KinematicCharacterController
{
    ...
    public struct KinematicCharacterMotorState
    {
        ...
        public RigidbodySynchronizable AttachedRigidbody;
    ...
    {
        public RigidbodySynchronizable RigidbodySynchronizable;
...
#pragma warning restore 0649
        ...
        [Header("RigidbodySynchronizable interaction settings")]
        /// <summary>
        ...
        /// <summary>
        /// Represents the RigidbodySynchronizable to stay attached to
        /// </summary>
        public RigidbodySynchronizable AttachedRigidbody { get { return _attachedRigidbody; } }
        private RigidbodySynchronizable _attachedRigidbody;
        /// <summary>
        ...
        /// <summary>
        /// Specifies a RigidbodySynchronizable to stay attached to
        /// </summary>
        [NonSerialized]
        public RigidbodySynchronizable AttachedRigidbodyOverride;
        /// <summary>
        ...
        private List<RigidbodySynchronizable> _rigidbodiesPushedThisMove = new List<RigidbodySynchronizable>(16);
        private RigidbodyProjectionHit[] _internalRigidbodyProjectionHits = new RigidbodyProjectionHit[MaxRigidbodyOverlapsCount];
        private RigidbodySynchronizable _lastAttachedRigidbody;
...
#pragma warning restore 0414 
        ...
        public void UpdatePhase1(float deltaTime)
        {
            ...
            if (InteractiveRigidbodyHandling)
            {
                #region Interactive RigidbodySynchronizable Handling 
                ...
                else
                {
                    ...
                    {
                        RigidbodySynchronizable interactiveRigidbody = GetInteractiveRigidbody(GroundingStatus.GroundCollider);
                ...
                // Movement from Attached RigidbodySynchronizable
                        ...
                        if (nbOverlaps > 0)
                        {
                            for (int i = 0; i < nbOverlaps; i++)
                            {
                                        ...
                                        out resolutionDistance))
                                {
                                    ...
                                    if (InteractiveRigidbodyHandling)
                                    {
                                        RigidbodySynchronizable probedRigidbody = GetInteractiveRigidbody(_internalProbedColliders[i]);
        ...
        /// </summary>
        private void StoreRigidbodyHit(RigidbodySynchronizable hitRigidbody, Vector3 hitVelocity, Vector3 hitPoint, Vector3 obstructionNormal, HitStabilityReport hitStabilityReport)
        {
            if (_rigidbodyProjectionHitCount < _internalRigidbodyProjectionHits.Length)
            {
                if (!hitRigidbody.GetComponent<KinematicCharacterMotor>())
                {
                    ...
                    rph.RigidbodySynchronizable = hitRigidbody;
        ...
        private void ProcessVelocityForRigidbodyHits(ref Vector3 processedVelocity, float deltaTime)
        {
            for (int i = 0; i < _rigidbodyProjectionHitCount; i++)
            {
                ...
                if (bodyHit.RigidbodySynchronizable && !_rigidbodiesPushedThisMove.Contains(bodyHit.RigidbodySynchronizable))
                {
                    if (_internalRigidbodyProjectionHits[i].RigidbodySynchronizable != _attachedRigidbody)
                    {
                        // Remember we hit this rigidbody
                        _rigidbodiesPushedThisMove.Add(bodyHit.RigidbodySynchronizable);

                        float characterMass = SimulatedCharacterMass;
                        Vector3 characterVelocity = bodyHit.HitVelocity;

                        KinematicCharacterMotor hitCharacterMotor = bodyHit.RigidbodySynchronizable.GetComponent<KinematicCharacterMotor>();
                        bool hitBodyIsCharacter = hitCharacterMotor != null;
                        bool hitBodyIsDynamic = !bodyHit.RigidbodySynchronizable.isKinematic;
                        float hitBodyMass = bodyHit.RigidbodySynchronizable.mass;
                        float hitBodyMassAtPoint = bodyHit.RigidbodySynchronizable.mass; // todo
                        Vector3 hitBodyVelocity = bodyHit.RigidbodySynchronizable.velocity;
                        ...
                        {
                            PhysicsMover physicsMover = bodyHit.RigidbodySynchronizable.GetComponent<PhysicsMover>();
                            if(physicsMover)
                            {
                        ...
                        {
                            bodyHit.RigidbodySynchronizable.AddForceAtPosition(velocityChangeOnBody, bodyHit.HitPoint, ForceMode.VelocityChange);
                        }
        ...
        {
            RigidbodySynchronizable colliderAttachedRigidbody = coll.attachedRigidbody;
        ...
        public void EvaluateHitStability(Collider hitCollider, Vector3 hitNormal, Vector3 hitPoint, Vector3 atCharacterPosition, Quaternion atCharacterRotation, Vector3 withCharacterVelocity, ref HitStabilityReport stabilityReport)
        {
            ...
            if (StepHandling != StepHandlingMethod.None && !stabilityReport.IsStable)
            {
                ...
                RigidbodySynchronizable hitRigidbody = hitCollider.attachedRigidbody;
        ...
        /// </summary>
        public void GetVelocityFromRigidbodyMovement(RigidbodySynchronizable interactiveRigidbody, Vector3 atPoint, float deltaTime, out Vector3 linearVelocity, out Vector3 angularVelocity)
        {
            if (deltaTime > 0f)
            {
        ...
        /// </summary>
        private RigidbodySynchronizable GetInteractiveRigidbody(Collider onCollider)
        {
            RigidbodySynchronizable colliderAttachedRigidbody = onCollider.attachedRigidbody;
            if (colliderAttachedRigidbody)
            {
